# ALPINE Repository Organization Summary
...
### ALPINE (Three Splits)
**Original**:
- `ALPINE/esm_2_peft_random.ipynb`
- `ALPINE/esm2_peft_cold.ipynb`
- `ALPINE/esm2_peft_seqsim.ipynb`

**New**:
- Config: `configs/alpine_config.yaml`
- Code: `train_alpine.py` + `--split random/cold_target/sequence_similarity`
...
- `BALMForLoRAFinetuning`: ALPINE with LoRA
...
# ALPINE (all splits)
for split in random cold_target sequence_similarity; do
    python train_alpine.py --config configs/alpine_config.yaml --split $split
done
```
...
alpine = pd.read_csv('results/alpine/cv_summary_metrics.csv')
...
comparison = pd.concat([
    ...
    alpine.rename(columns={'Mean': 'Alpine_Mean', 'Std Dev': 'Alpine_Std'})
], axis=1)
```
...
git init
git add .
git commit -m "Organize ALPINE experiments for GitHub"

# Add remote
git remote add origin https://github.com/yourusername/ALPINE.git
...
1. **Supplementary Code**: Link to `ALPINE_organized` GitHub repository
